#!/bin/bash
# Complex-quantized RPFQ-Swin training script.
...
OUTPUT_DIR="./output/rpfq-swin-tiny-q2a4"
EXPERIMENT="rpfq-swin-tiny-q2a4"
mkdir -p "${OUTPUT_DIR}"
...
"${PYTHON_BIN}" -m torch.distributed.run --nproc_per_node="${NPROC_PER_NODE:-8}" scripts/train.py "${IMAGENET1K_DIR}" \
  --model rpfq_swin_tiny_patch4_window7_224_q \
